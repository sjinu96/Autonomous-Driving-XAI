...
def explain(path, eps):
    ...
    for idx, class_name in enumerate(class_names):
        img=plt.imread(os.path.join(test_dir, class_name, os.listdir(os.path.join(test_dir, class_name))[positive_best[idx]]))
        label=idx
...
def train(path, **training_options):
    ...
    print('main.train 실행')
    print('train option : ', training_options)
    ds_train, ds_test = _get_dataset()
    ...
    for epoch in range(max(TRAIN_EPOCHS)):
        ...
        print(path+'{}'.format(epoch)+'.pt', 'save...') # 0604
        _model_save(path+'{}'.format(epoch), m_orig, training_options)
...
def adv_images(m, images, labels, opts):
    if opts.steps <= 0:
        return images
...
def model_evaluate(m, ds_loader):
...
def model_robust_get_grads(guesses, images, labels, train):
...
def model_robust_get_psi(robust_integration):
    return ROBUST_ADAPT_PSI_0 * math.exp(ROBUST_ADAPT_PSI * robust_integration) # ri가 증가하지 않으면 psi_0=30, psi=0.02 고정, 즉 증가함.


def model_save(path, m, training_options): # 0604 
    torch.save({'model_params': m.state_dict(),
            'training_options': training_options}, path+'.pt')
...
def model_train_batch(m, training_options, opt, images, labels):
    stats = {}
    ...
    if training_options['robust_additions']:
        ...
        ri += max(-ROBUST_ADAPT_EPS_NEG, min(ROBUST_ADAPT_EPS_POS, # 0604 neg : 0.01, pos : 1 # 지수적으로 증가한다. 0.01과 1중 최대니 (1)일 것.
                -math.log(loss.item() / ROBUST_ADAPT_L_TARGET))) # l_target : 1.5
        ri = max(0., ri)
